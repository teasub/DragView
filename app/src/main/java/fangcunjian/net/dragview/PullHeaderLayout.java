...
import fangcunjian.net.dragview.internal.ElasticOutInterpolator;
import fangcunjian.net.dragview.internal.SimpleAnimatorListener;
...
public class PullHeaderLayout extends ViewGroup implements NestedScrollingParent, NestedScrollingChild {
    ...
    public static final int STATE_FLING = 2;//手指离开屏幕但屏幕还在移动
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (mHeaderTopView != null){
            measureChildWithMargins(mHeaderTopView,widthMeasureSpec, 0, heightMeasureSpec, 0);
        }
    ...
    private void layoutChildren() {
        ...
        if (mHeaderTopView != null){
            MarginLayoutParams lp = (MarginLayoutParams) mHeaderTopView.getLayoutParams();
            final int left = paddingLeft + lp.leftMargin;
            final int top = 0;
            final int right = left + mHeaderTopView.getMeasuredWidth();
            final int bottom = DEFAULT_SHRINK;
            mHeaderTopView.layout(left, top, right, bottom);
        }
    ...
    private void setHeaderBarView(){
        LinearLayout linearLayout = new LinearLayout(getContext());
        linearLayout.setBackgroundResource(R.mipmap.ic_launcher);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,  DEFAULT_SHRINK);
        if (mHeaderTopView != null){
            mHeaderTopView = null;
        }
        mHeaderTopView = linearLayout;
        addView(mHeaderTopView,params);
        mHeaderTopView.setVisibility(GONE);
    }

    private void showOrHideTopView(){
        int scroolY = DEFAULT_HEIGHT - DEFAULT_SHRINK;
        if (null != mHeaderController){
            if (mHeaderController.getScroll() < scroolY && mHeaderController.getScroll() > 80){
                mHeaderTopView.setAlpha(0.0f);
                alphaAnimation(mHeaderTopView, 1f, 0f);
                mHeaderTopView.setVisibility(GONE);
            }
            if (mHeaderController.getScroll() == scroolY && !mHeaderController.canScrollUp()){
                alphaAnimation(mHeaderTopView, 0f, 1f);
                mHeaderTopView.setVisibility(VISIBLE);
            }

            if (mHeaderController.getCurPosition() != DEFAULT_SHRINK){
                alphaAnimation(mHeaderTopView, 1f, 0f);
                mHeaderTopView.setVisibility(GONE);
            }
    ...
    private void alphaAnimation(View view, float alpha1, float alpha2){
        if (null != view){
            UIUtils.clearAnimator(view);
            ObjectAnimator anim = ObjectAnimator.ofFloat(view, "alpha", alpha1, alpha2);
